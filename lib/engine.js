...
module.exports = function (s, conf) {
  ...
  if (so.strategy) {
    let str_path
    try {
      str_path = path.resolve(so.strategy)
      s.strategy = require(str_path)
      console.log('load strategy from ' + str_path)
    } catch (e) {
      str_path = path.resolve(__dirname, `../extensions/strategies/${so.strategy}/strategy`)
      s.strategy = require(str_path)
      console.log('load strategy from ' + str_path)
    }
